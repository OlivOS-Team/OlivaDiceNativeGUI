...
class Event(object):
    ...
    def init_after(plugin_event: OlivOS.API.Event, Proc: OlivOS.pluginAPI.shallow):
        ...
        OlivaDiceCore.crossHook.dictHookList['model'].append([
            'OlivaDiceNativeGUI',
            OlivaDiceNativeGUI.data.OlivaDiceNativeGUI_ver_short,
        ])
        ...
        threading.Thread(target=OlivaDiceNativeGUI.load.checkOnlineStatusLoop, args=()).start()
            ...
            if plugin_event.data.event == 'OlivaDiceNativeGUI_001':
                if not OlivaDiceNativeGUI.load.flag_open or True:
                    ...
                    OlivaDiceNativeGUI.GUI.ConfigUI(
                        Model_name='OlivaDiceNativeGUI_manage', logger_proc=Proc.Proc_info.logger_proc.log
                    ).start()
